...
execute if entity @e[tag=side_start,tag=searching] at @e[tag=helper_1] unless entity @e[tag=cell,distance=..1] as @e[tag=helper_2] at @s unless entity @e[tag=cell,distance=..1] if score distX vars < target_size settings run function mn:maze_gen/cell/skin/regular/5x10/west
# One free space
...
execute at @e[tag=helper_1] unless entity @e[tag=cell,distance=..1] at @e[tag=helper_2] unless entity @e[tag=cell,distance=..1] if score @e[tag=helper_2,limit=1] dist_frm_strt > target_size settings as @e[tag=helper_1] at @s run function mn:maze_gen/cell/edge/west/determine_corner
...
execute at @e[tag=helper_1,tag=dWest] unless entity @e[tag=cell,distance=..1] at @e[tag=helper_2] unless entity @e[tag=cell,distance=..1] if score distX vars = target_size settings as @e[tag=helper_1] at @s run function mn:maze_gen/cell/skin/regular/straight_ew
execute at @e[tag=helper_1,tag=dWest] unless entity @e[tag=cell,distance=..1] at @e[tag=helper_2] unless entity @e[tag=cell,distance=..1] if score distX vars = target_size settings as @e[tag=helper_1] at @s run setblock ~ ~1 ~ redstone_block
execute at @e[tag=helper_1,tag=dWest] unless entity @e[tag=cell,distance=..1] at @e[tag=helper_2] unless entity @e[tag=cell,distance=..1] if score distX vars = target_size settings as @e[tag=helper_2] at @s run function mn:maze_gen/cell/edge/west/determine_corner


execute if score @e[tag=helper_1,limit=1] dist_frm_strt > target_size settings run say Something has gone very wrong [West]
...
# continue execution?
execute if entity @e[tag=helper_1,tag=dWest] at @e[tag=helper_2] run tp @e[tag=helper_1] ~-5 ~ ~
execute if entity @e[tag=helper_1,tag=dWest] if entity @e[tag=side_start,tag=searching] run function mn:maze_gen/cell/edge/west/start